...
static error_code stack_recalloc(ident_stack_t* stack, size_t new_capacity) {
    ...
    LOGGER_DEBUG("Trying to realloc %lu bytes", (new_capacity ON_STACK_CANARY_DEBUG(+ 2)) * sizeof(var_st_type));
    var_st_type* new_block = (var_st_type*)realloc(stack->original_ptr, (new_capacity ON_STACK_CANARY_DEBUG(+ 2)) * sizeof(var_st_type));
    ...
    var_st_type* new_data = new_block;
    ON_STACK_CANARY_DEBUG(
